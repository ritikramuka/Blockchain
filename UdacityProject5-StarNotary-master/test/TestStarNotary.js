...
it('can add the star name and star symbol properly', async() => {
  ...
  await instance.createStar('Hello Star!', tokenId, {from: accounts[0]})
    assert.equal(await instance.name(), 'Indian Token');
    assert.equal(await instance.symbol(), 'INT');
...
it('lets a user transfer a star', async() => {
    ...
    let tokenId = 26;
    let instance = await StarNotary.deployed();
    await instance.createStar('Hello tokenID 26 Star!', tokenId, {from: accounts[1]});
...
it('lookUptokenIdToStarInfo test', async() => {
  let instance = await StarNotary.deployed();
  await instance.createStar('Awesome Star!', 33, {from: accounts[2]});
  assert.equal(await instance.lookUptokenIdToStarInfo(33), 'Awesome Star!');
  });
